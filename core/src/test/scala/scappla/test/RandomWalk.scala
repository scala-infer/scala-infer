...
class RandomWalkSpec extends FlatSpec {
  ...
  it should "fit volatility" in {
    ...
    val hidden = (0 until 20).scanLeft(0.0) { case (v, _) =>
    ...
    // val volGuide = ReparamGuide(Normal(Param(0.0), exp(Param(0.0))))
    // val errMu = Param(-2.0)
    // val errSigma = exp(Param(0.0))
    // val errGuide = ReparamGuide(Normal(errMu, errSigma))
    val lambdaGuide = ReparamGuide(Normal(Param(0.0), exp(Param(0.0))))

    val dataGuideErr = exp(Param(-1.0))
    val dataWithGuides = data.map { v =>
      val guide = ReparamGuide(Normal(Param(v), dataGuideErr))
      (v, AutoRegressive(guide, 1.0))
    }

    val volParam = exp(Param(log(0.1)))
    val errParam = exp(Param(log(0.2)))
    val model = infer {
      // val vol = exp(sample(Normal(0.0, 1.0), volGuide))
    ...
    val interpreter = new OptimizingInterpreter(new Adam(0.1))
    for { iter <- 0 until 10000 } {
      ...
      if (iter % 100 == 0) {
        ...
        println(s"RANDOMWALK: ${iter}, ${vol}, ${err}, ${lambda.v}")
      }
      interpreter.reset()
    }
