...
object IndexOf {
  ...
  implicit def indexOfHListCase0[X <: Dim[_]]: Aux[X, X, _0] =
    new IndexOf[X, X] {
      type Out = _0
      def apply() = Nat._0
      def toInt = 0
  ...
  implicit def indexOfHListCase1[T <: Shape, X <: Dim[_]]: Aux[X :#: T, X, _0] =
    new IndexOf[X :#: T, X] {
      type Out = _0
      def apply() = Nat._0
      def toInt = 0
  ...
  implicit def indexOfHListCaseN[T <: Shape, H <: Dim[_], X <: Dim[_], I <: Nat]
